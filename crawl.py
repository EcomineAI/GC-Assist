...
def build_chunks(results):
    ...
    for r in results:
        ...
        ts      = r.get("crawled_at", datetime.utcnow().isoformat() + "Z")
...
def main():
    ...
    # ── Save raw KB ──
    raw_path = OUTPUT_DIR / "knowledge_base_raw.json"
    with open(raw_path, "w", encoding="utf-8") as f:
        json.dump(chunks, f)
    print(f"  [OK] knowledge_base_raw.json — {raw_path.stat().st_size / 1024:.1f} KB")
